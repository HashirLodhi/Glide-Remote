...
class MainActivity : AppCompatActivity() {
 ...
 private var selecting=false; private var dragging=false
 ...
 override fun onPause(){endDrag();scrollPending=0f;pendingX=0f;pendingY=0f;points.clear();super.onPause()}
 ...
 private fun buildUi(){
  ...
  selection.addView(button("SELECT TEXT",false){v->endDrag();selecting=!selecting;(v as Button).text=if(selecting)"SELECTING ON" else "SELECT TEXT"},LinearLayout.LayoutParams(0,dp(48),1f))
  listOf("copy" to "COPY", "paste" to "PASTE").forEach{(key,title)->selection.addView(button(title,false){endDrag();send("key",key)},LinearLayout.LayoutParams(0,dp(48),1f).apply{setMargins(dp(6),0,0,0)})}
  root.addView(selection)
  root.addView(label("Position cursor, enable SELECT TEXT, then drag to highlight. Lift to finish. Copy uses your PC clipboard.",11f,muted,false).apply{setPadding(0,dp(8),0,dp(8))})
  ...
  root.addView(button("BACKSPACE",false){
   val start=minOf(entry.selectionStart,entry.selectionEnd).coerceAtLeast(0)
   val end=maxOf(entry.selectionStart,entry.selectionEnd).coerceAtLeast(0)
   if(end>start)entry.text.delete(start,end)
   else if(start>0)entry.text.delete(Character.offsetByCodePoints(entry.text,start,-1),start)
   else if(entry.text.isEmpty())send("key","backspace")
 ...
 private fun flushMove(){val x=pendingX;val y=pendingY;pendingX=0f;pendingY=0f;if(abs(x)+abs(y)>.08f)ws?.send(JSONObject().put("type","move").put("dx",x).put("dy",y).toString())}
 private fun mouseButton(action:String){ws?.send(JSONObject().put("type","button").put("button","left").put("action",action).toString())}
 private fun endDrag(){if(dragging){flushMove();mouseButton("up");dragging=false}}
 ...
 private fun touch(view:View,e:MotionEvent):Boolean{
  ...
  when(e.actionMasked){
   MotionEvent.ACTION_DOWN->{moved=false;maxPointers=1;scrollRemainder=0f;remember();if(selecting){flushMove();mouseButton("down");dragging=true}}
   MotionEvent.ACTION_POINTER_DOWN->{endDrag();maxPointers=maxOf(maxPointers,e.pointerCount);remember()}
   ...
   MotionEvent.ACTION_UP->{val wasDragging=dragging;endDrag();if(!moved&&!wasDragging){send("click",if(maxPointers>1)"right" else "left");view.performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP)};points.clear()}
   MotionEvent.ACTION_CANCEL->{endDrag();points.clear();scrollPending=0f}
  }
  return true
 }
 ...
 override fun onDestroy(){endDrag();ws?.close(1000,"App closed");client.dispatcher.executorService.shutdown();super.onDestroy()}
}
