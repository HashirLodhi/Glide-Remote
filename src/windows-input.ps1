...
public static class RemoteInput {
 ...
 public static void Shortcut(ushort key) {
  try { Submit(new INPUT[] { VirtualKey(0x11,0), VirtualKey(key,0), VirtualKey(key,2) }); }
  finally { Submit(new INPUT[] { VirtualKey(0x11,2) }); }
 }
...
while(($line=[Console]::In.ReadLine())-ne $null){try{$m=$line|ConvertFrom-Json
 ...
 elseif($m.type-eq'button'-and$m.button-eq'left'){if($m.action-eq'down'){[RemoteInput]::mouse_event(2,0,0,0,[UIntPtr]::Zero)}elseif($m.action-eq'up'){[RemoteInput]::mouse_event(4,0,0,0,[UIntPtr]::Zero)}}
 ...
 elseif($m.type-eq'key'-and$m.key-eq'copy'){[RemoteInput]::Shortcut(0x43)}
 elseif($m.type-eq'key'-and$m.key-eq'paste'){[RemoteInput]::Shortcut(0x56)}
